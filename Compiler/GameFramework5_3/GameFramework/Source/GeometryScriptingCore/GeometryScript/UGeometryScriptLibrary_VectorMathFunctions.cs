...
public partial class UGeometryScriptLibrary_VectorMathFunctions : UBlueprintFunctionLibrary {
	...
	///<summary>Returns a Scalar List constructed with each element is the product (ConstantMultiplier * A * B)</summary>
	...
	///<summary>Compute (ConstantMultiplier * A * B)  where A and B are the corresponding elements from ScalarListA and ScalarListB accordingly, and store the</summary>
	...
	///<summary>Returns a Scalar List of the same length as the input Scalar List, with the elements computed as (Constant * A) where A is the corresponding element in the input List.</summary>
	...
	///<summary>Compute (Constant * A) for each element A is the Scalar List, and the result is stored in the original Scalar List.</summary>
